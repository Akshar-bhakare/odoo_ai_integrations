...
import { fetchIncentiveActuals } from '@/lib/accounting/incentive-actuals';
...
export async function GET(request: NextRequest) {
  return incentiveRoute(request, {}, async (actor) => {
    ...
    if (!/^\d+$/.test(rawPage) || Number(rawPage) < 1) throw new Error('Page must be a positive integer');
    return fetchIncentiveActuals({
      gateway: defaultOdooGateway,
      companyId: actor.currentCompanyId,
      dateFrom,
      dateTo,
      salesperson: request.nextUrl.searchParams.get('salesperson') ?? undefined,
      page: Number(rawPage),
    });
  });
}
